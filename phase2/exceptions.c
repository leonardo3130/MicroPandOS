...
static void passUpOrDie(int i, state_t *exception_state) {
  if (current_process) { // per sicurezzza
    ...
    if (current_process->p_supportStruct != NULL) {
      saveState(&(current_process->p_supportStruct->sup_exceptState[i]),
                exception_state); // salvataggio stato
...
static void syscallExceptionHandler(state_t *exception_state) {
  if ((exception_state->status << 30) >>
      31) { // controllo che il processo non sia in kernel-mode
    // scrivo come codice dell'eccezione il valore EXC_RI e invoco il Pass Up or
    // Die
    exception_state->cause =
        (exception_state->cause & CLEAREXECCODE) | (EXC_RI << CAUSESHIFT);
    ...
    if (exception_state->reg_a0 == SENDMESSAGE) {
      ...
      int ready;      // se uguale a 1 il destinatario è nella ready queue, 0
                      // altrimenti
      ...
      LDST(exception_state); // QUI CAMBIA DA CURRPROCESS CON PID 1 A CURR
                             // PROCESS CON PID
    }
    // SYS2
    else if (exception_state->reg_a0 == RECEIVEMESSAGE) {
      struct list_head *msg_inbox =
          &(current_process->msg_inbox);           // inbox del ricevente
      unsigned int from = exception_state->reg_a1; // da chi voglio ricevere
      msg_t *msg = popMessage(
          msg_inbox,
          (from == ANYMESSAGE ? NULL
                              : (pcb_t *)(from))); // rimozione messaggio dalla
                                                   // inbox del ricevente
      ...
      if (!msg) {
        ...
        // accedo all'area di memoria in cui andare a caricare il payload del
        // messaggio
