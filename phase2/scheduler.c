...
void scheduler() {
  //  1. Se c'e' almeno un processo pronto ad essere eseguito, lo prendo dalla
  //  Ready e lo assegno al current_process.
  ...
  if (current_process != NULL) {
    ...
    // nel caso la Ready Queue dovesse risultare vuota si entra in questo ramo
    // else in cui avviena la deadlock detection
    if (process_count == 1) {
      HALT();
