...
static void blockProcessOnDevice(pcb_t *p, int line, int term) {
  ...
  case IL_PRINTER:
    ...
    break;
  case IL_TERMINAL:
    insertProcQ(((term > 0) ? &Locked_terminal_transm : &Locked_terminal_recv),
                p);
    break;
  default:
    break;
...
/*  Funzione per la creazione di un nuovo processo come figlio del processo
 * sender  */
...
void ssi_terminate_process(pcb_t *proc) {
  if (!(proc == NULL)) {
    // ciclo in cui iterativamente si scorre la lista dei figli e chiama la
    // funzione ricorsivamente sui fratelli
  ...
  // Se il processo si trova in una delle liste dei processi bloccati dei
  // dispositivi, decremento il soft_blocked_count
...
/*  Funzione per il clockwait che incrementa il soft_blocked_count e inserisce
 * il sender nella lista Locked_pseudo_clock */
void ssi_clockwait(pcb_t *sender) {
  soft_blocked_count++;
  insertProcQ(&Locked_pseudo_clock, sender);
}

/*  Funzione che ritorna il pid del sender se arg = NULL, altrimenti il pid del
 * parent */
int ssi_getprocessid(pcb_t *sender, void *arg) {
  return (arg == NULL ? sender->p_pid : sender->p_parent->p_pid);
}

/*  Funzione che incrementa il soft_blocked_count, chiama la funzione
   addrToDevice() passando commandAddr e imposta quest'ultimo uguale al valore
   del campo commandValue  */
...
/*  Funzione che gestisce mediante il payload ricevuto dal sender la richiesta
 * di un servizio   */
unsigned int SSIRequest(pcb_t *sender, ssi_payload_t *payload) {
  unsigned int ret = 0;
  switch (payload->service_code) {
  case CREATEPROCESS:
    ret = (emptyProcQ(&pcbFree_h)
               ? NOPROC
               : (unsigned int)ssi_new_process(
                     (ssi_create_process_PTR)payload->arg, sender));
    break;
