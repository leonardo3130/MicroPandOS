...
void initPcbs() {
  for (int i = 0; i < MAXPROC; i++) {
    freePcb(&(pcbTable[i])); // list_add_tail(&(pcbTable[i].p_list),
                             // &pcbFree_h);
  }
}

// mette l’elemento puntato da p nella lista dei processi liberi;
void freePcb(pcb_t *p) { list_add_tail(&(p->p_list), &pcbFree_h); }
...
// se la coda la cui testa è puntata da head è vuota ritrna TRUE, altrimeti
// FALSE
...
pcb_t *removeProcQ(struct list_head *head) {
  if (emptyProcQ(head))
    ...
    pcb_t *first_pcb =
        container_of(list_next(head), pcb_t,
                     p_list);       // salvo un puntatore all'elemento in testa
...
pcb_t *outProcQ(struct list_head *head, pcb_t *p) {
  ...
  list_for_each(iter, head) {
    q = container_of(iter, pcb_t,
                     p_list); // prendo l'elemento dell'iterazione corrente
    if (q == p) {             // se corrisponde a p lo rimuovo e lo ritorno
      list_del(&(q->p_list));
      return p;
    }
  }
  return NULL; // se la lista è estata iterata completamenete e non è stato
               // trovato p ritorno NULL
}

int emptyChild(pcb_t *p) {
  return list_empty(&(p->p_child)); // controllo che la lista dei figli sia
                                    // vuota
}
...
void insertChild(pcb_t *prnt, pcb_t *p) {
  ...
  if (emptyChild(prnt))
    ...
    list_add_tail(
        &(p->p_sib),
        &(prnt->p_child)); // aggiungo in coda per rispettare la condizione FIFO
}
...
pcb_t *removeChild(pcb_t *p) {
  if (emptyChild(p))
    ...
    pcb_t *first_child = container_of(list_next(&(p->p_child)), pcb_t,
                                      p_sib); // ricerca primo figlio
    list_del(&(first_child->p_sib)); // rimozione figlio dalla lista dei
                                     // fratelli
